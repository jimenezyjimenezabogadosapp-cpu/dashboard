...
export default function ReporteriaGeneralClient() {
    ...
    useEffect(() => {
        async function fetchData() {
          ...
          SELECT 
            (SELECT COUNT(*) FROM client_tbl) as busquedas_realizadas,
            (SELECT COUNT(*) FROM alert_tbl) as total_alertas,
            (SELECT COUNT(DISTINCT client_id) FROM alert_tbl) as clientes_con_alertas,
            (SELECT AVG(execute_time) FROM stadistics_usage_tbl) as tiempo_promedio_caso
        `));
          ...
          SELECT 
            (SELECT COUNT(*) FROM client_tbl) as "Total clientes",
            (SELECT COUNT(DISTINCT client_id) FROM alert_tbl) as "Total clientes con alertas"
        `));
        ...
        `));
                ...
                const kpis: KpiData[] = [
                    {
                        ...
                        value: kpiData[0]?.busquedas_realizadas || 0
                    },
                    {
                        label: "Total alertas",
                        value: kpiData[0]?.total_alertas || 0
                    },
                    {
                        label: "Clientes con alertas",
                        value: kpiData[0]?.clientes_con_alertas || 0
                    },
                    {
                ...
                const processedRiskDetailsRows = Array.isArray(riskDetailsData) ? riskDetailsData : [];
                setRiskDetailsRows(processedRiskDetailsRows);
